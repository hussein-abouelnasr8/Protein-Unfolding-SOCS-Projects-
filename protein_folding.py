...
data = np.load("force_extension_data_NIck.npz", allow_pickle = True)
positions2 = np.array([np.array(x, dtype=float) for x in data['final_positions']])
...
F_pull_list_kjmol = np.array([5, 5.5 , 6, 6.5 , 7, 7.5 , 8, 8.5, 9,9.5,10,11, 12, 13, 14, 15, 16, 18, 20, 30, 40])
F_pull_list_kjmol = F_pull_list_kjmol[::-1] 
...
for F_index in range(len(F_pull_list)):
    ...
    for time_step in range(int(time_steps)):
      F = total_force_contribution(positions2,
            idx_pull, k_trap, r_trap0, v_pull,
            angle_triple_indices, k_theta, theta_eq,
            angle_quadruple_indices, k_phi, phi_eq,
            k_r, r_eq, keys, hydroph_m, radius, residue_names)
      ...
      positions2, velocities = baoab_step_pull(positions2, velocities, masses_md, dt, gamma, T, F, F_pull, time, pull_period)
      planar_angles = planar_bond_angles(positions2, angle_triple_indices, degrees=True)
      di_angles = dihedral_angles(positions2, angle_quadruple_indices, degrees = True)
      time += dt
      # if int(time)%10 == 0:

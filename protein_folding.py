...
def langevin_step(positions, gamma, dt, kB, T, total_force, N_a, x_pull):
    """
    positions : (N,3)
    velocities: (N,3)
    masses    : (N,)
    ...
    positions_new = positions[:-1,:] + (dt/ g[:-1,:]) * total_force[:-1,:] + W1[:-1,:]
    positions_new.append(positions_new, x_pull, axis = 0)
...
for t in range(int(time_steps)):
  ...
  F = total_force_contribution(positions, t,
        ...
        k_r, r_eq, keys, hydroph_m, radius, residue_names)
  x_pull = F_pull(t, dt, v_pull, x_min, x_max)
